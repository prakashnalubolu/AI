...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for _ in range(self.iterations):
            # Copy the current values to avoid in-place updates
            ...
            for state in self.mdp.getStates():
                if not self.mdp.isTerminal(state):
                    ...
                    for action in self.mdp.getPossibleActions(state):
                        # Compute the Q-value for the state-action pair
                        q_value = self.computeQValueFromValues(state, action)
                        if q_value > max_value:
                            max_value = q_value
                    # Update the new value for the state
                    new_values[state] = max_value
            # Update the current values to the newly computed values
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, prob in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            # Compute the Q-value based on the Bellman equation
